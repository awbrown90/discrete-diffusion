...
def run(max_steps=100000,lr=1e-4,seed=0):
    ...
    for s0 in (T1,T2,T3):
        for a in (0,1,2):
            s1 = map_transition(s0,a)
            s0_a = copy.deepcopy(s0)
            s0_a[-1] = actions[a]
            s1[-1] = actions[1]
            examples.append((torch.concatenate([s0_a,s1])))
    ...
    try:
        for step in range(1, max_steps+1):
            ...
            idx = torch.randperm(len(examples))      # e.g. tensor([2,0,3,1])
            for i in idx:
                clean = examples[i]
            ...
            if step % 5 == 0:
                ...
                if interactive:
                    ...
                    if chosen_action is not None:
                        ...
                        live[0,8] = actions[chosen_action]

            if not interactive and step % 10 == 0:
                chosen_action = torch.randint(0, 3, (1,)).item()
                print('auto regressive update')
                live = torch.cat([live[0,9:], torch.randint(0,84,(9,)).to(dev)]).unsqueeze(0)
                live[0,8] = actions[chosen_action]
