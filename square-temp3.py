...
#----------------------------------------------------------------------------
corner_TL3=pt([[1,1,0],[1,0,0],[0,0,0]])
corner_TR3=pt([[0,1,1],[0,0,1],[0,0,0]])
corner_BR3=pt([[0,0,0],[0,0,1],[0,1,1]])
corner_BL3=pt([[0,0,0],[1,0,0],[1,1,0]])
...
T2=torch.tensor([
    ...
    horizontal,vertical,horizontal,
    horizontal,vertical,horizontal],dtype=torch.long)
T3=torch.tensor([
...
class STModel(nn.Module):
    def __init__(self, d=256, heads=4, num_layers=3, T=3):
        super().__init__()
        self.d = d
...
# pre–define your 4 (src,tgt) pairs once
triplets = [
    (torch.cat([T1, T2, T3]), torch.cat([T1, T2, T3])),
    (torch.cat([T2, T3, T2]), torch.cat([T2, T3, T2])),
    (torch.cat([T3, T2, T1]), torch.cat([T3, T2, T1])),
    (torch.cat([T2, T1, T2]), torch.cat([T2, T1, T2])),
]
...
def run(max_steps=100000,lr=1e-4,seed=0):
    ...
    live = torch.cat([torch.randint(0,84,(9,)), torch.randint(0,84,(9,)),torch.randint(0,84,(9,))]).unsqueeze(0).to(dev)
    learning = True
    generate = True
    try:
        for step in range(1, max_steps+1):
            ...
            idx = torch.randperm(len(triplets))      # e.g. tensor([2,0,3,1])
            for i in idx:
                clean, _ = triplets[i]
                src_batch.append(noisy_sample(clean))
                tgt_batch.append(clean)

            src = torch.stack(src_batch, dim=0).to(dev)   # (4, 3*9)
            tgt = torch.stack(tgt_batch, dim=0).to(dev)   # (4, 3*9)
            logits = m(src)
            ...
            if step % 5 == 0:
                ...
                vis = np.hstack([to_img(live[0,18:]) ])
            ...
            if step % 10 == 0:
                ...
                prev12 = live[0, 9:27]            # keep frames 2&3
                live = torch.cat([prev12, torch.randint(0,84,(9,)).to(dev)]).unsqueeze(0)
                #live = torch.cat([live[0,9:18], live[0,18:],torch.randint(0,84,(9,)).to(dev)]).unsqueeze(0)
