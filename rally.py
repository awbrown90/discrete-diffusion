...
GRID_H = 2  # number of rows
GRID_W = 3  # number of columns
# -------------------------------------------------------------------

# Turn this on to paint an extra horizontal bar at (static_r, static_c)
USE_HORIZONTAL = True
random_miss=False
r_paddle = 0
static_r, static_c = 0, 1  # row, col for the horizontal bar when enabled
d_c = 1
...
class RallyEnv:
    def __init__(self, device, grid_h=2, grid_w=3, random_miss=True):
        self.r_paddle = 0
        self.grid_h = grid_h
        self.grid_w = grid_w
        self.ball_r = 0
    ...
    def getState(self):
        current_state = torch.full((GRID_H*GRID_W,), zero_idx,
                               dtype=torch.long, device=self.device)
        current_state[self.r_paddle * self.grid_w + 0] = vertical_idx
        
        if self.ball_idx == 2:
            if self.random_miss:
                # fill every cell with a random pattern ID 0…len(PATTERNS)-1
                ...
                current_state[self.ball_r * self.grid_w + self.ball_c] = horizontal_idx
        else:
            current_state[self.ball_r * self.grid_w + self.ball_c] = balls[self.ball_idx]
    ...
    def step(self, action: int) -> torch.Tensor:
        ...
        new = torch.full((self.grid_h * self.grid_w,), zero_idx,
                        dtype=torch.long, device=self.device)

        # paint vertical bar in col 0
        new[self.r_paddle * self.grid_w + 0] = vertical_idx

        
        self.ball_c += self.ball_d_c
        if self.ball_c >= self.grid_w-1:
            if self.ball_d_c == 0:
                self.ball_r = (self.ball_r+1)%2
            self.ball_d_c -= 1
        elif self.ball_c <= 1:
            if self.ball_d_c == 0 and self.ball_idx != 2:
                if self.r_paddle != self.ball_r: #then ball miss
                    self.ball_idx = 2
                    self.miss_counter += 1
                    self.ball_d_c -= 1
            self.ball_d_c += 1
        ...
        if self.ball_idx == 2:
            if self.random_miss:
                ...
                new[self.ball_r * self.grid_w + self.ball_c] = horizontal_idx
        else:
            new[self.ball_r * self.grid_w + self.ball_c] = balls[self.ball_idx]
...
ball_idx = 0
balls = [uphorizontal_idx, dnhorizontal_idx, horizontal_idx]
...
def run(max_steps=50000, lr=1e-3, seed=0):
    ...
    #_     = load_checkpoint(model, opt, 'ckpt.pth', dev)
    ...
    buffer = ReplayBuffer(observation_shape=(extW,), capacity=50_000, device=dev)

    # initial state: vertical at row 0, col 0 (and maybe horiz)
    '''
    current_state = torch.full((GRID_H*GRID_W,), zero_idx,
                               dtype=torch.long, device=dev)
    current_state[0] = vertical_idx
    if USE_HORIZONTAL:
        current_state[static_r*GRID_W + static_c] = balls[ball_idx]
    '''

    random_mode = False

    env_train = RallyEnv(dev, random_miss=random_mode)
    env_test = RallyEnv(dev, random_miss=random_mode)
        ...
        for step in range(1, max_steps+1):
            ...
            if step % 4 == 0:
                print(f"step {step} | loss {loss.item():.4f} | "
                      f"learning={learning} | interactive={interactive} | col {static_c}")
                ...
                elif interactive and key in [82,84]:
                    # up/down arrow → pick action manually
                    chosen = 0 if key==82 else 1
                    live = torch.cat([
                        live[0, extW:],
