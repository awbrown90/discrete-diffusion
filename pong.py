...
class PongEnv:
    def __init__(self, device, grid_h=5, grid_w=7, random_miss=True):
        self.r_paddle = 0
        self.grid_h = grid_h
        self.grid_w = grid_w
        self.ball_r = 0
    ...
    def step(self, action: int) -> torch.Tensor:
        ...
        if self.ball_c >= self.grid_w-1:
            ...
            if self.ball_d_c == 0 and self.ball_idx != 4:
        ...
        # if self.ball_idx != 4:
        if self.ball_d_c == 1 and self.ball_d_r == -1:
            self.ball_idx = 0
        elif self.ball_d_c == -1 and self.ball_d_r == -1:
            self.ball_idx = 1
        elif self.ball_d_c == 1 and self.ball_d_r == 1:
            self.ball_idx = 2
        elif self.ball_d_c == -1 and self.ball_d_r == 1:
...
def run(max_steps=50000, lr=1e-3, seed=0):
    ...
    _     = load_checkpoint(model, opt, 'pong_ckpt.pth', dev)
    ...
    try:
        ...
        save_model = True
        if save_model:
            save_checkpoint(model, opt, step, 'pong_ckpt.pth')
        if save_log:
            if random_mode:
