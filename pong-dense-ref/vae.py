...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_vq = use_vq
        self.use_variational = False     # you disabled it already
        ...
        if not use_vq:
            ...
            self.quant_conv      = nn.Linear(enc_dim, latent_dim)  # pre-quant
            self.post_quant_conv = nn.Linear(latent_dim, dec_dim)
    ...
    def encode(self, x):
        ...
        if not self.use_vq:
            m = self.quant_conv(x)        # (B, seq_len, mult*latent)
            if not self.use_variational:
                m = torch.cat([m, torch.zeros_like(m)], dim=-1)
            return DiagonalGaussianDistribution(m, deterministic=not self.use_variational, dim=2)
        else:
            return self.quant_conv(x)     # pre-quant

          
    def quantize(self, z_e):
        z_q, idx_map, vq_loss = self.vq(z_e)       # order from the library
        return z_q, idx_map, vq_loss              # <-- idx_map second
    ...
    def unpatchify(self, x):
        ...
        x = x.view(b,3,p,p,self.seq_h,self.seq_w).permute(0,1,4,2,5,3).reshape(b,3,self.seq_h*p,self.seq_w*p)
        return x
    ...
    def forward(self, x, sample_posterior: bool = True):
        ...
        # --- forward()  VQ branch  -----------------------------------------
            ...
            # 1) encode → (B, N, D) **already passed through self.quant_conv**
            z_e = self.encode(x)

            # 2) quantize
            z_q, idx_map, vq_loss_base = self.quantize(z_e)   # note variable rename
...
def ViT_L_20_Shallow_Encoder(**kwargs):
    return AutoencoderKL(
        latent_dim=128,
        input_height=64,  input_width=64,  patch_size=8,
        enc_dim=256, enc_depth=4, enc_heads=8,
