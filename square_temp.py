...
class Model(nn.Module):
    def __init__(self,d=128,layers=4,heads=8):
        ...
        self.pos = nn.Parameter(torch.randn(18,d)*0.02)    # 18 positions now
        self.enc = nn.TransformerEncoder(nn.TransformerEncoderLayer(d,heads,batch_first=True),layers)
        self.head= nn.Linear(d,84)
    def forward(self,x):                    # (B,18)
...
def run(max_steps=100000,lr=1e-4,seed=0):
    ...
    m2=Model_org().to(dev); opt2=torch.optim.AdamW(m2.parameters(),lr=lr)
    ...
    try:
        for step in range(1, max_steps+1):
            ...
            src2   = torch.cat([src0, src1], dim=0).to(dev)   # (2,9)
            tgt2   = torch.cat([T1.unsqueeze(0), T2.unsqueeze(0)], 0).to(dev)  # (2,9)
            ...
            if learning:
                ...
                loss2 = F.cross_entropy(logits2.view(-1,84), tgt2.view(-1))
            ...
            if generate:
                with torch.no_grad():
                    # 1. use single-frame model on the *first* 9 tokens
                    logits_f0 = m2(live[:, :9])                     # (1, 9, 84)
                    probs_f0  = F.softmax(logits_f0, -1)
                    live[:, :9] = torch.multinomial(probs_f0[0], 1).squeeze(1)

                    # 2. use two-frame model on the whole sequence, take future frame
                    logits_2f  = m(live)                           # (1, 18, 84)
                    probs_2f   = F.softmax(logits_2f, -1)
                    next_f1    = torch.multinomial(probs_2f[0], 1).squeeze(1)[9:]  # (9,)
                    live[:, 9:] = next_f1
