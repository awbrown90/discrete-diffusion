...
# Training schedule
WARMUP_STEPS     = 20_000     # VQ-only first, then joint
VQ_BATCH_SIZE    = 32
IMG_BUF_CAP      = 512
TOK_BUF_CAP      = 1000
ST_BATCH         = 32
SEQ_MIN          = 4  # Updated for longer temporal window
SEQ_MAX          = 4  # Fixed to T=4 for capturing velocity
...
# ===================== ENV =====================
class PongEnv:
    def __init__(self, device, grid_h=GRID_H, grid_w=GRID_W, patch_size=PATCH, ball_size=5.0, paddle_height=20.0, speed=2.0, random_miss=True):
        self.grid_h     = grid_h
        self.grid_w     = grid_w
        self.patch_size = patch_size
        self.ball_size  = ball_size
        self.paddle_height = paddle_height
        self.paddle_width = 5.0
        self.speed      = speed
        self.height     = grid_h * patch_size
        self.width      = grid_w * patch_size
        self.device     = device
        self.random_miss = random_miss
        self.paddle_y   = (self.height - self.paddle_height) / 2  # start middle
        self.reset_ball()

    def reset_ball(self):
        self.ball_x = self.width / 2
        self.ball_y = random.uniform(0, self.height - self.ball_size)
        self.vx = self.speed
    ...
    def render(self):
        ...
        # paddle
        py0 = int(self.paddle_y)
        img[py0:py0 + int(self.paddle_height), 0:int(self.paddle_width)] = (0, 255, 0)
        # ball
        bx0 = int(self.ball_x)
        by0 = int(self.ball_y)
        img[by0:by0 + int(self.ball_size), bx0:bx0 + int(self.ball_size)] = (0, 0, 255)
    ...
    def step(self, action: int):
        # paddle move
        if action == 0:  # up
            self.paddle_y = max(0, self.paddle_y - self.speed)
        elif action == 1:  # down
            self.paddle_y = min(self.height - self.paddle_height, self.paddle_y + self.speed)
        # action 2: stay
        ...
        if self.ball_y < 0:
            ...
            self.ball_y = 2 * (self.height - self.ball_size) - self.ball_y
            self.vy = -self.vy

        # bounce right
        if self.ball_x > self.width - self.ball_size:
            self.ball_x = 2 * (self.width - self.ball_size) - self.ball_x
            self.vx = -self.vx

        # left: paddle bounce or miss
        if self.ball_x < self.paddle_width:
            if self.paddle_y < self.ball_y + self.ball_size and self.ball_y < self.paddle_y + self.paddle_height:
                # bounce
                self.ball_x = 2 * self.paddle_width - self.ball_x
                self.vx = -self.vx
                # add angle based on hit position
                hit_pos = (self.ball_y - self.paddle_y) / self.paddle_height - 0.5
                self.vy += hit_pos * self.speed
            else:
                # miss
                if self.random_miss and random.random() < 0.1:  # optional random miss
                    pass
                self.reset_ball()
...
def run():
    ...
    start_step = 0
    if LOAD_CKPT:
        ckpt_path = 'ckpts/joint_ckpt2.pth'
    ...
    # Initialize live with 4 consecutive real frames for T=4
    live_frames = []
    current_state = env_test.getState()
    img = current_state.float() / 255.0
    _, _, idx_map = vq(img.unsqueeze(0))
    z = idx_map.squeeze(0)
    action = random.randint(0, len(actions)-1)
    ext = extend_with_action_col(z, actions[action], zero_idx)
    live_frames.append(ext)
    for _ in range(3):
        next_state = env_test.step(action)
        img = next_state.float() / 255.0
        _, _, idx_map = vq(img.unsqueeze(0))
        z = idx_map.squeeze(0)
        action = random.randint(0, len(actions)-1)
        ext = extend_with_action_col(z, actions[action], zero_idx)
        live_frames.append(ext)
    live = torch.cat(live_frames).unsqueeze(0).to(dev)
    ...
    try:
        for step in range(start_step + 1, MAX_STEPS + 1):
            ...
            if learning and len(tok_buf) >= 64:
                ...
                if step < WARMUP_STEPS:
                    ...
                    # Display GT and recon for current frame during warmup
                    if SHOW_GUI and step % 2 == 0:
                        gt_img = (img01.permute(1,2,0).cpu().numpy() * 255).clip(0,255).astype(np.uint8)
                        gt_img = cv2.cvtColor(gt_img, cv2.COLOR_RGB2BGR)
                        cv2.imshow('GT Sample', gt_img)

                        with torch.no_grad():
                            recon_cur, _, _ = vq(img01.unsqueeze(0))
                        recon_img = (recon_cur.squeeze(0).permute(1,2,0).cpu().numpy() * 255).clip(0,255).astype(np.uint8)
                        recon_img = cv2.cvtColor(recon_img, cv2.COLOR_RGB2BGR)
                        cv2.imshow('VQ Recon Sample', recon_img)
                        cv2.waitKey(1)
            ...
            if generate:
                with torch.no_grad():
                    ...
                    # For T=4, overwrite the last frame only
                    last_frame_start = frame_size_ext * 3
                    live[:, last_frame_start:] = preds[:, last_frame_start:]

            # --- display (same style as working script) ---
            if SHOW_GUI and (step % 2 == 0):
                with torch.no_grad():
                    # Display the first and last frame for simplicity
                ...
                if   key == 27: break
                    ...
                    live = torch.cat([
                        torch.randint(0, VOCAB_SIZE, (frame_size_ext,)) for _ in range(4)
                    ]).unsqueeze(0).to(dev)
                elif key == ord('m'):
                    ...
                    save_checkpoint(st, vq, opt_st, opt_vq, tok_buf, img_buf, step, 'ckpts/joint_ckpt2.pth')
                # --- interactive arrow keys: Up(82), Down(84), Right(83) ---
                    ...
                    # Autoregress: shift live left by one frame, append new state_ext
                    live = torch.cat([
            ...
            if not interactive and step % 10 == 0:
                ...
                # Autoregress: shift left, append new
                live = torch.cat([
                    live[0, frame_size_ext:],
                    state_ext
                ]).unsqueeze(0)
        ...
        if SHOW_GUI:
            try:
                ...
                pass
        if SAVE_CKPT_EVERY:
            save_checkpoint(st, vq, opt_st, opt_vq, tok_buf, img_buf, step, 'ckpts/joint_last2.pth')
